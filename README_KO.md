...
- **Slack이 곧 인터페이스** — 채널이나 DM에서 @멘션으로 Claude Code 실행
- **채널별 프로젝트 매핑** — 채널마다 다른 프로젝트 디렉토리 연결
- **Socket Mode** — 공인 IP나 ngrok 불필요
- **쉘 인젝션 방지** — `child_process.spawn` 사용, 쉘 실행 없음
- **스마트 메시지 처리** — 긴 응답 자동 분할, 마크다운→텍스트 변환
- **안전장치 내장** — 이중 타임아웃, 중복 방지, 사용자 권한 제어, 무한 루프 방지
